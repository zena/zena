  ...
  def self.up
    ...
    Site.all.each do |site|
      ...
      if ref.new_record?
        ...
        contact = secure(VirtualClass) do
        ...
        if contact.new_record?
          ...
          execute "UPDATE nodes SET vclass_id = #{contact.id} WHERE kpath = 'NRC' AND site_id = #{site.id}"
        end
      end

      # Add first_name, name and email to Contact vclass
      TRANSFER_KEYS.each do |key|
        Column.create(:role_id => contact.id, :ptype => 'string', :name => key)
      end

      # Move first_name, name, email from User to Contact
      User.find(:all, :conditions => ["site_id = ?", site.id]).each do |user|
        if contact = user.contact
          contact.update_attributes(Hash[*(TRANSFER_KEYS.map {|k| [k, user[k]]}).flatten])
        end
      end
    end
